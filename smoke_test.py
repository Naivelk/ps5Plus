...
def probar_config():
    ...
    for clave in ("palabras_psplus", "palabras_suscripcion",
                  "palabras_excluir", "palabras_excluir_titulo",
    ...
    revisar(cfg.get("mi_region") in ("US", "CO"),
            "mi_region es US o CO", "-> %r" % cfg.get("mi_region"))
...
def probar_latido(cfg):
    ...
    revisar(latido.toca(cfg_latido, None, domingo), "en domingo: manda")
    revisar(not latido.toca(cfg_latido, domingo.isoformat(), domingo),
            "si ya se mandó hoy: no repite (48 pasadas al día)")
    revisar(not latido.toca(cfg_latido, None, datetime.date(2026, 7, 29)),
            "un miércoles: no manda")
    revisar(not latido.toca({"activo": False, "dia": 6}, None, domingo),
